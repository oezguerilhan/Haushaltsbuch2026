...
function parseDeutscheBank(items) {
  ...
  const bareAmountRe = /^(\d{1,3}(?:\.\d{3})*,\d{2})$/;
  ...
  while (i < rows.length) {
    ...
    if (/^Deutsche Bank|^Filiale|^Lindenallee|^Herr|^Telefon|^24h-|^Kontoauszug|^Kontoinhaber|^Auszug|^Seite|^IBAN|^Alter Saldo|^Neuer Saldo|^Buchung|^Valuta|^Vorgang|^Soll|^Haben|^0000000|^Kundennummer|^Gebuchte|^Zeitraum|^Konto \(|^Kontostand|^von \d|^DE\d{2}\s/i.test(rowText)) {
      i++; continue;
    }
    ...
    if (amount === 0) {
      for (let j = 0; j < allItems.length - 1; j++) {
        if (allItems[j].text === '-' || allItems[j].text === '+') {
          const numMatch = allItems[j + 1]?.text.match(bareAmountRe);
          if (numMatch) {
    ...
    // Check for bare amounts without +/- sign (Haben in newer DB format)
    if (amount === 0) {
      for (const it of allItems) {
        const m = it.text.match(bareAmountRe);
        if (m && !datePartRe.test(it.text) && !yearRe.test(it.text)) {
          amount = parseFloat(m[1].replace(/\./g, '').replace(',', '.'));
          type = 'income'; // No sign = Haben (credit)
        }
      }
    }
      ...
      for (const it of nextItems) {
        if (amount === 0) {
          ...
          if (m) {
            ...
            const bm = it.text.match(bareAmountRe);
            if (bm && !datePartRe.test(it.text) && !yearRe.test(it.text)) {
...
function parseDeutscheBankOnline(items) {
  ...
  const skipRe = /^Kundennummer|^Gebuchte|^Zeitraum|^Konto \(|^Kontostand|^von \d|^DE\d{2}\s|^Buchung\b|^Wertstellung|^Umsatz\b|^Soll\b|^Haben\b|^Währung/i;
  ...
  while (i < rows.length) {
    ...
    // Check the date row itself for amounts (online format has everything on one line)
    for (const it of rows[i].items) {
      if (foundAmount) break;
      const sm = it.text.match(signedAmtRe);
      if (sm) { amount = Math.abs(parseFloat(sm[1].replace(/\./g, '').replace(',', '.'))); type = 'expense'; foundAmount = true; }
      else {
        const bm = it.text.match(bareAmtRe);
        if (bm && !fullDateRe.test(it.text)) { amount = parseFloat(bm[1].replace(/\./g, '').replace(',', '.')); type = 'income'; foundAmount = true; }
      }
    }
    // Also collect description from the date row (e.g. "SEP A Lastschrift")
    for (const it of rows[i].items) {
      if (!fullDateRe.test(it.text) && !signedAmtRe.test(it.text) && !bareAmtRe.test(it.text) && it.text !== 'EUR' && it.text !== '-') {
        descParts.push(it.text);
      }
    }
